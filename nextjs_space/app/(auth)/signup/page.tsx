  ...
  return (
    <Card>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div>
          <label htmlFor="email" className="block text-sm font-medium text-hill-text mb-2">Email</label>
          <input id="email" type="email" value={email}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setEmail(e?.target?.value ?? '')}
            required className="w-full bg-hill-gray border border-hill-border rounded-lg px-4 py-3 text-hill-white placeholder-hill-muted focus:outline-none focus:border-hill-orange transition-colors"
            placeholder="you@example.com" />
        </div>

        <div>
          <label htmlFor="password" className="block text-sm font-medium text-hill-text mb-2">Password</label>
          <input id="password" type="password" value={password}
            onChange={(e: React.ChangeEvent<HTMLInputElement>) => setPassword(e?.target?.value ?? '')}
            required minLength={6} className="w-full bg-hill-gray border border-hill-border rounded-lg px-4 py-3 text-hill-white placeholder-hill-muted focus:outline-none focus:border-hill-orange transition-colors"
            placeholder="Min 6 characters" />
        </div>

        <div>
          <label htmlFor="confirmPassword" className="block text-sm font-medium text-hill-text mb-2">Confirm Password</label>
        ...
        <Button type="submit" loading={loading} className="w-full">Create Account & Continue to Payment</Button>
      ...
      <div className="mt-6 text-center">
        <p className="text-hill-muted text-sm">
          ...
          <Link href="/login" className="text-hill-orange hover:underline">Sign in</Link>
        </p>
      </div>
    </Card>
  )
